...
from boardapp.models import Subscribers, Post
...
def my_job():
    ...
    subject = f'Weekly newsletter;'
    text = '\n'.join(
        ['{}'.format(p.title) + f' Link: http://127.0.0.1:8000{p.get_absolute_url()}' for p in posts])
    for u in users:
        email = u.user.email
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            my_job,
            trigger=CronTrigger(minute="00", hour="18", day_of_week="fri"),  # Every 10 seconds
